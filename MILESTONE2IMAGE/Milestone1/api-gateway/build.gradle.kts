...
dependencies {
    ...
    // ─ JaCoCo agent (so integration tests also get instrumented)
    testRuntimeOnly("org.jacoco:org.jacoco.agent:${jacoco.toolVersion}")
}

tasks.test {
    useJUnitPlatform()
    // generate the report right after tests finish
...
tasks.jacocoTestReport {
    dependsOn(tasks.test) // tests have to run before we can report
    reports {
...
tasks.jacocoTestCoverageVerification {
    ...
    violationRules {
        rule {
            // you can scope by element or counter if you like; this checks overall coverage
            limit {
